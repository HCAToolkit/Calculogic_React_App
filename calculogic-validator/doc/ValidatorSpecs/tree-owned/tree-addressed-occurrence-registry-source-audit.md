...
| Folder-kind evidence helper | `calculogic-validator/tree/src/tree-folder-kind-evidence.logic.mjs` | Registry-backed folder-kind interpretation from structural-home and semantic-home evidence. | Emits `structural` when structural-home evidence exists; `semantic` when semantic-home evidence exists; otherwise `unspecified`, limited to registered folder kinds. | `tree-structure-advisor.wiring.mjs`; output consumed by occurrence-classification replacement runtime and parity evidence. | Already represented and actively consumed; now receives Tree-owned semantic repository-top-home evidence for `calculogic-validator` and `calculogic-doc-engine` before occurrence classification. | Reusable for addressed occurrences as an interpreted Tree evidence layer; adapter should preserve its evidence-only status. |
| Semantic repository-top homes built-in registry | `calculogic-validator/tree/src/registries/_builtin/semantic-repository-top-homes.registry.json` | Tree-owned semantic repository-top-home vocabulary for known semantic package roots. | `calculogic-doc-engine`, `calculogic-validator`, with status and definition fields. | Loaded by `getBuiltinSemanticRepositoryTopHomesRegistry`; passed by tree wiring into `prepareTreeSemanticHomeEvidence`; consumed downstream by folder-kind evidence and occurrence-classification replacement runtime. | Current runtime truth: these package roots are semantic repository-top homes, not structural homes, while repo-shape policy separately allows their folder names at repository top. | Reusable only for direct repo-top semantic-home evidence; it must not classify descendants by first path segment. |
...
| Semantic-home evidence helper | `calculogic-validator/tree/src/tree-semantic-home-evidence.logic.mjs` | Bounded Tree evidence join from addressed occurrences to Naming-prepared semantic evidence plus Tree-owned semantic repository-top-home evidence. | Uses Naming-owned `semanticName`, `semanticFamily`, `familyRoot`, and optional `familySubgroup` by path/type match; also consumes `semanticRepositoryTopHomes[]` for direct repo-top folder matches and emits `semanticHome` as evidence. | `tree-structure-advisor.wiring.mjs`; output consumed by folder-kind evidence, occurrence-classification replacement runtime, and parity evidence. | Current runtime truth: Tree-owned semantic repo-top evidence exists for direct repo-top `calculogic-validator` and `calculogic-doc-engine` folder occurrences and coexists with Naming-derived evidence without re-owning Naming fields. | Useful as supporting evidence only; semantic family/name fields remain Naming-owned and should not become Tree fields. Descendants such as `calculogic-validator/tree` and `calculogic-validator/tree/src` are not repository-top homes. |
...
Current implementation reality: `tree-semantic-home-evidence.logic.mjs` joins addressed occurrences to Naming-prepared semantic evidence directly and separately consumes the dedicated semantic repository-top homes registry. It still does not consume `semantic-home-policy.registry.json`.
...
7. **Tree now has a verified active source that classifies `calculogic-validator` and `calculogic-doc-engine` as semantic repository-top homes in the downstream occurrence-classification path.** The dedicated semantic repository-top homes registry is Tree-owned; Naming-backed semantic-home evidence remains related evidence and is not automatically equivalent to Tree-owned semantic repository-top-home classification.
...
2. **How does the structural-homes registry currently affect that folder?**
   - `prepareTreeStructuralHomeEvidence` builds a lookup from `structuralHomes[]` and emits structural-home evidence only when a folder occurrence path is itself a single repo-top token matching that registry. Therefore `src`, `doc`, and `scripts` currently produce structural-home evidence when present as repo-top folder occurrence paths. `calculogic-validator` and `calculogic-doc-engine` do not produce structural-home evidence because they are intentionally absent from `structural-homes.registry.json`.
3. **How does current semantic-home evidence classify repository-top folders, if at all?**
   - `prepareTreeSemanticHomeEvidence` has a Tree-owned semantic repository-top-home registry for direct repo-top folder occurrences and also joins addressed occurrence records to Naming-prepared semantic evidence by path/type. `calculogic-validator` and `calculogic-doc-engine` become semantic in the replacement runtime through Tree-owned semantic repository-top-home evidence; this does not turn every non-structural allowed repo-top folder into a semantic repo-top home.
...
5. **Are `calculogic-validator` and `calculogic-doc-engine` currently producing incorrect structural-home evidence?**
   - No in current runtime truth. They have been retired from `structural-homes.registry.json`; the corrected current runtime truth is that they are allowed semantic repository-top homes, not structural repository-top homes.
...
7. **Would removing the two entries cause them to become semantic correctly, become unspecified, disappear from classification, or change finding behavior?**
   - The entries are removed after semantic repository-top classification is available and wired into the same downstream path. `repo-shape-policy.registry.json` still allows both names for fallback behavior, while the prepared replacement route now classifies those repo-top records as semantic with `isRepoShapeAllowedTopLevelDirectory: true`.
...
9. **Are lower-level folders safely distinguished from repo-top homes?**
   - Yes in the audited helpers. `prepareTreeStructuralHomeEvidence` only accepts paths without `/`, and replacement classification only applies repo-top root classification when `resolvedPath` has no `/`. Current tests assert that `calculogic-validator/tree` and `calculogic-validator/tree/src` are not repo-top occurrences and remain `unclassified`.
...
| non-repository-top descendant | Addressed occurrence `resolvedPath`/lineage/depth from snapshot; repo-top classification requires no `/` in `resolvedPath` | `calculogic-validator/tree`, `calculogic-validator/tree/src` | Not repo-top; no structural-home evidence from repo-top registry; replacement classification leaves `calculogic-validator/tree` unclassified in scoped-root tests, and treats descendants separately from repo-top homes. | Continue to be descendants, not independent repository-top homes; no reuse of repo-top-home rule below repository top. | Snapshot/address evidence consumers; replacement classification for non-root metadata only. | No registry change; future adapters must preserve the repo-top guard. |
...
- preserve the repo-top-only guard and do not classify descendants such as `calculogic-validator/tree` or `calculogic-validator/tree/src` as repository-top homes;
