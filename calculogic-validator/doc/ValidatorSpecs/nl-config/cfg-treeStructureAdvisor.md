...
- Tree naming semantic-family advisory details now expose optional Tree-owned `treeStructuralContext` on valid prepared occurrence-local evidence while keeping Addressing source-trace facts and Naming qualification metadata separate; no new Tree structural judgment policy is introduced.
