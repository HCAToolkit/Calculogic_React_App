...
- Tree-owned semantic repository-top-home evidence is prepared for known semantic package roots including `calculogic-validator` and `calculogic-doc-engine`; those roots are current runtime truth as semantic repository-top homes, not structural repository-top homes.
- Repo-shape allowance is not semantic or structural repository-top classification: repo-shape policy allows top-level folder names, while Tree-owned evidence classifies a repo-top occurrence as structural, semantic, or `unspecified`. `unspecified` is the fallback state when stronger Tree-owned evidence is absent, not the intended state for known allowed semantic repository-top homes.
