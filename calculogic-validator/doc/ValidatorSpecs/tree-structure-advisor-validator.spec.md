...
Current implementation reality includes a guarded Tree-owned prepared-evidence join helper for callers that explicitly provide both the staged Naming occurrence bridge and addressed occurrence records under the same full identity tuple. The helper may copy Naming semantic fields and Naming-owned ambiguity/split-family flags from bridge observations, but it must not re-derive Naming semantic-family interpretation from filenames. Missing, invalid, mismatched, ambiguous, diagnostic-bearing, empty, or no-match identity state produces explicit skipped/no-evidence status/diagnostics instead of clean joins.
