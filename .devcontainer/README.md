...
- `@calculogic/report-capture` and the scripts `addressing:get-tree`, `report:verify`, `report:summarize`, and `report:examples:validator` invoke embedded `calculogic-validator/scripts/*.host.mjs` files directly by path, never through the installed `@calculogic/validator` package. This live-development workflow does not migrate them, and they are genuinely unaffected by linking or unlinking `@calculogic/validator`.
- The `validate:naming:validator:*` scripts (`:entry`, `:naming`, `:tree`, `:doc`) are different: they invoke the `calculogic-validate-naming` **executable**, which resolves through `node_modules/.bin` and therefore _does_ become the linked standalone implementation in live mode. Only their `--target calculogic-validator/...` arguments still point at the embedded tree. In live mode, these scripts run the linked standalone checkout's naming logic against the embedded tree's files — do not treat their output as a stable/embedded control while linked.
