...
- Public API boundary for host consumers is `@calculogic/doc-engine` only; Host runtime code imports from `@calculogic/doc-engine` package root only (no deep imports).
...
- Registry behavior tests use local mock providers and depend only on `@calculogic/doc-engine` (package root) and local mock providers.
